...
<body>
  ...
  <div class="footer">
    Rankings-driven MVP. Favorite-team bump persists in your browser and only changes via the selector.
  </div>
...
function populateSelector(selectEl, teams, defaultTeam) {
  ...
  // Always include defaultTeam first
  const ordered = [];
  ordered.push(defaultTeam);

  // Then all teams except duplicates of default
  teams.forEach(t => {
...
function render(byDay, cfg, favorite) {
  ...
  days.forEach(day => {
    ...
    if (games.length === 0) {
      sec.innerHTML += `<div class="empty">No ranked games found.</div>`;
...
async function init() {
  ...
  const defaultFav = cfg.default_favorite_team || "Virginia";
  const savedFav = localStorage.getItem(STORAGE_KEY);
  const favorite = savedFav || defaultFav;

  document.getElementById("subline").textContent =
    `Generated ${generated} • Times shown in Mountain Time • Favorite: ${favorite}`;
  ...
  // Ensure selection is in dropdown
  const options = Array.from(select.options).map(o => o.value);
  const useFav = options.includes(favorite) ? favorite : defaultFav;

  select.value = useFav;
  localStorage.setItem(STORAGE_KEY, useFav);
